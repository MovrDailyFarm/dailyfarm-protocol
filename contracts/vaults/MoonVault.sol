...
contract MoonVault is BaseVault {
    ...
    function _harvest() internal override {
        ...
        if (moonAmount > 0) {
            ...
            if (path0.length > 0) {
                IUniswapV2Router01(moonRouter).swapExactTokensForTokens(moonAmount.div(2), 1, path0, address(this), block.timestamp);
            }
            if (path1.length > 0) {
                IUniswapV2Router01(moonRouter).swapExactTokensForTokens(moonAmount.div(2), 1, path1, address(this), block.timestamp);
            }
            IUniswapV2Router01(moonRouter).addLiquidity(token0, token1, 
